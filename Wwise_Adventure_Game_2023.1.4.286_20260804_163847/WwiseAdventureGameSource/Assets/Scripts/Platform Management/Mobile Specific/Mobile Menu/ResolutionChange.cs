...
public class ResolutionChange : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        dropdown.captionText.text = (currentResolution.width + " × " + currentResolution.height + (isMobile ? "" : " @ " +currentResolution.refreshRateRatio));

        for (int i = 0; i < resolutions.Length; i++)
        {
            Resolution r = resolutions[i];
            Dropdown.OptionData option = new Dropdown.OptionData((r.width + " × " + r.height + (isMobile ? "" : " @ " + r.refreshRateRatio)));
