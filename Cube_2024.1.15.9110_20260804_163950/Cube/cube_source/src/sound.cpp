...
void snd_registptr( void * ent, char * in_name )
{
	...
	if ( in_name == NULL )
	{
		...
		char name[256];
		snprintf( name, 256, "Entity #%i", (int) ++cEntity );
...
void snd_init()
{
	...
	// Initialize sound engine.
	if ( SoundEngine::Init( &initSettings, &platInitSetings ) != AK_Success )
	{
...
static void pitchYawToOrientation(float in_pitch, float in_yaw, AkWorldTransform& out_transform)
{
	float fRadPitch = rad(in_pitch);
	float fCosPitch = cosf(fRadPitch);
	float fSinPitch = sinf(fRadPitch);

	float fRadYaw = rad(in_yaw - 90);
	float fCosYaw = cosf(fRadYaw);
	float fSinYaw = sinf(fRadYaw);
...
static void baseentToAkTransform(baseent * in_ent, AkWorldTransform& out_transform)
{
	pitchYawToOrientation(in_ent->pitch, in_ent->yaw, out_transform);
...
void snd_setpos( void * ent, float x, float y, float z, float pitch, float yaw )
{
	CHECK_SOUND_ENGINE;

	AkSoundPosition snd;

	pitchYawToOrientation(pitch, yaw, snd);
